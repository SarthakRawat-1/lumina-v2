"""
Code Checker - ESLint validation for React/JSX code

This module validates AI-generated React components using ESLint
to ensure they can be safely executed in the frontend.
"""
import re
import subprocess
import json
...
# Plugin imports that are added to the code before validation
PLUGIN_IMPORTS = """
...
def find_react_code_in_response(text: str) -> Optional[str]:
    """
    ...
    def extract_balanced_braces(text: str, start_pos: int) -> tuple[Optional[str], int]:
    ...
    def extract_function_body(text: str, start_pos: int) -> Optional[str]:
...
class ESLintValidator:
    ...
    def __init__(self, eslint_base_dir: Optional[str] = None):
        """
        ...
        # ESLint executable - handle Windows vs Unix
        ...
        for required_path in [self.config_file_path, self.package_json_path, self.node_modules_path]:
    ...
    def validate_jsx(self, jsx_code: str) -> dict:
        """
        Validates JSX using ESLint.
    ...
    def _parse_eslint_output(self, eslint_json_output: str) -> dict:
        """Parse ESLint JSON output into a structured result"""
        try:
...
# Test function
def _test_validator():
    """Test the ESLint validator"""
    # Example of code with a linting error
    jsx_code_with_error = """
